...
def add_correlated_differences(alpha=0.5):
    ...
    for subset in xrange(0, 10):
            ...
            for b in (f for f in train.columns if dtypes[f] != 'object'):
                if a != b:
                    ...
                    if new_feature not in corrs:
                        ...
                        sparse_b = train[b][train[b] != -1]
                        corr = sparse_a.corr(sparse_b)  # min_periods?
                        if corr > alpha:
                            print a, b, corr
        ...
        result = pd.concat([train, new_train], axis=1)
        result.to_csv(path.join(directory, 'corr-' + str(alpha) + dataset.TRAIN_FILE))

        result = pd.concat([test, new_test], axis=1)
        result.to_csv(path.join(directory, 'corr-' + str(alpha) + dataset.TEST_FILE))


add_correlated_differences()
